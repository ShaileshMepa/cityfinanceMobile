...
export class Configure extends Component {
  ...
  _changeEyeIcon = () => {
    const { show, visible1 } = this.state;
    this.setState({
      show: !show,
      visible1: !visible1,
    });
  ...
  async componentDidMount() {
    ...
    console.log("Getting ASuth>>>", AuthToken);
  ...
  GeneratePinAPI = () => {
    if (this.state.pinset === "") {
    ...
    } else if (this.state.confirmpinset === "") {
      alert("Please Provide confirmpin!");
    } else {
      fetch("https://cityfinance-app.herokuapp.com/api/update-users-pin", {
        method: "POST",
        headers: new Headers({
          "Content-Type": "application/json", // <-- Specifying the Content-Type
        ...
        .then(async (responseText) => {
          ...
          if (responseText.success === true) {
            this.props.navigation.navigate("PinAuth");
      ...
      // fetch("https://cityfinance-app.herokuapp.com/api/generatePIN", {
      //   method: "POST",
      //   headers: new Headers({
      //     "Content-Type": "application/json", // <-- Specifying the Content-Type
      //     Authorization: "Bearer " + this.state.authToken,
      ...
      //     confirmpin: this.state.confirmpinset,
      //   }),
      // })
      //   .then((response) => response.json())
      //   .then((responseText) => {
      ...
      <ScrollView style={{ flex: 1, backgroundColor: "#10AFA6" }}>
        ...
        >
          ...
          <View style={{ padding: 10 }}>
            ...
            >
              <TextInput
                onChangeText={(text) => this.setState({ pinset: text })}
          ...
          <View style={{ padding: 10 }}>
            ...
            >
              <TextInput
                onChangeText={(text) => this.setState({ confirmpinset: text })}
